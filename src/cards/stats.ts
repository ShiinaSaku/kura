...
const createTextNode = ({
  ...
  const kValue =
    numberFormat.toLowerCase() === "long" || id === "prs_merged_percentage"
      ? value
      : kFormatter(typeof value === "number" ? value : Number(value), precision);
...
const renderStatsCard = (stats: StatsData, options: Partial<StatCardOptions> = {}): string => {
  const {
    name,
    totalStars,
    totalCommits,
  ...
  const isLongLocale = typeof locale === "string" ? LONG_LOCALES.includes(locale) : false;
