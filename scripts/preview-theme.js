...
import { isValidHexColor, isValidGradient } from "../src/common/color";
...
const THEME_PR_SUCCESS_TEXT = ":heavy_check_mark: Theme PR does adhere to our guidelines.";
const FAIL_TEXT = `
  \rUnfortunately, your theme PR contains an error or does not adhere to our [theme guidelines](https://github.com/shiinasaku/github-readme-stats/blob/master/CONTRIBUTING.md#themes-contribution). Please fix the issues below, and we will review your\
...
const THEME_CONTRIB_GUIDELINES = `
  \rHi, thanks for the theme contribution. Please read our theme [contribution guidelines](https://github.com/shiinasaku/github-readme-stats/blob/master/CONTRIBUTING.md#themes-contribution).

  \r> [!WARNING]\
  \r> Keep in mind that we already have a vast collection of different themes. To keep their number manageable, we began to add only themes supported by the community. Your pull request with theme addition will be merged once we get enough positive feedback from the community in the form of thumbs up :+1: emojis (see [#1935](https://github.com/shiinasaku/github-readme-stats/issues/1935#top-themes-prs)). We expect to see at least 10-15 thumbs up before making a decision to merge your pull request into the master branch. Remember that you can also support themes of other contributors that you liked to speed up their merge.
  ...
  \r> [!NOTE]\
  \r> Also, note that if this theme is exclusively for your personal use, then instead of adding it to our theme collection, you can use card [customization options](https://github.com/shiinasaku/github-readme-stats#customization).
`;
const COLOR_PROPS = {
  title_color: 6,
  icon_color: 6,
...
const findComment = async (octokit, issueNumber, owner, repo, commenter) => {
  ...
  for await (const { data: comments } of octokit.paginate.iterator(
    ...
    const comment = comments.find((comment) => isPreviewComment(inputs, comment));
    if (comment) {
 ...
 */
const upsertComment = async (octokit, issueNumber, repo, owner, commentId, body) => {
  let resp;
 ...
 */
const addReview = async (octokit, prNumber, owner, repo, reviewState, reason) => {
...
const getGRSLink = (colors) => {
  const url = `https://api.kmi.moe/api?username=shiinasaku`;
...
const parseJSON = (json) => {
  try {
    ...
    if (typeof parsedJson === "object") {
      ...
      throw new IncorrectJsonFormatError("PR diff is not a valid theme JSON object.");
    }
  } catch (error) {
    ...
    try {
      ...
      throw new IncorrectJsonFormatError(`Theme JSON file could not be parsed: ${error.message}`);
    }
  }
};
...
export const run = async () => {
  try {
    ...
    const comment = await findComment(OCTOKIT, PULL_REQUEST_ID, OWNER, REPO, commenter);
    ...
    if (Object.keys(themeObject).every((key) => typeof themeObject[key] !== "object")) {
    ...
    const themeValid = Object.fromEntries(Object.keys(themeObject).map((name) => [name, true]));
    let previewBody = "";
    for (const theme in themeObject) {
      ...
      if (colors) {
        const missingKeys = REQUIRED_COLOR_PROPS.filter((x) => !Object.keys(colors).includes(x));
        const extraKeys = Object.keys(colors).filter((x) => !ACCEPTED_COLOR_PROPS.includes(x));
        if (missingKeys.length > 0 || extraKeys.length > 0) {
          ...
          for (const [colorKey, colorValue] of Object.entries(colors)) {
            if (colorValue[0] === "#") {
              errors.push(`Theme color property \`${colorKey}\` should not start with '#'`);
              invalidColors = true;
      ...
      if (invalidColors) {
        themeValid[theme] = false;
        previewBody += `
          \r### ${themeName.charAt(0).toUpperCase() + themeName.slice(1)} theme preview
      ...
      Object.keys(colorPairs).forEach((item) => {
        ...
        if (!ccc.isLevelAA(`#${color1}`, `#${color2}`)) {
          ...
          warnings.push(`\`${item}\` does not pass [AA contrast ratio](${permalink})`);
          themeValid[theme] = false;
        }
      });
    ...
    const reviewReason = themesValid ? undefined : INVALID_REVIEW_COMMENT(comment_url);
    if (DRY_RUN) {
      info(`DRY_RUN: Review state: ${reviewState}`);
      info(`DRY_RUN: Review reason: ${reviewReason}`);
    } else {
      await addReview(OCTOKIT, PULL_REQUEST_ID, OWNER, REPO, reviewState, reviewReason);
      await addRemoveLabel(OCTOKIT, PULL_REQUEST_ID, OWNER, REPO, "invalid", !themesValid);
    }
  } catch (error) {
    ...
    if (DRY_RUN) {
      ...
      await addRemoveLabel(OCTOKIT, PULL_REQUEST_ID, OWNER, REPO, "invalid", true);
    }
